...
<html lang="en">
    ...
    <body>
        <div class="container">
            ...
            @if(isset($perkiraan) && count($perkiraan) > 0)
                @foreach ($perkiraan as $child)
                    <table class="table table-bordered mt-1">
                        <tr>
                            <th>Periode Kehamilan</th>
                            <th>:</th>
                            <th>{{ $child['fields']['periode_kehamilan']['stringValue'] }}</th>
                        </tr>
                        <tr>
                            <td>Nama Istri</td>
                            <td>:</td>
                        ...
                        <tr>
                            <td>Riwayat Medis</td>
                            <td>:</td>
                            <td>{{ $child['fields']['riwayat_medis']['stringValue'] }}</td>
                        </tr>
                        <tr>
                            <td>Tanggal Haid Terakhir</td>
                            <td>:</td>
                            <td>{{ (new \DateTime($child['fields']['tanggal_haid']['stringValue']))->format('d F Y') }}</td>
                        </tr>
                        <tr>
                            <td>Tanggal Perkiraan Kelahiran</td>
                            <td>:</td>
                            <td>{{ (new \DateTime($child['fields']['perkiraan_kelahiran']['stringValue']))->format('d F Y') }}</td>
                        </tr>
                    </table>
                @endforeach
            @else
