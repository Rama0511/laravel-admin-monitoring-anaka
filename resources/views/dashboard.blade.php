...
<body>
    <nav class="navbar navbar-expand-lg navbar-light">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('perkembangan.create') }}">    Data Perkembangan Anak   </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('consultation.index') }}">    Konsultasi   </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('imunisasi.index') }}">   Imunisasi   </a>
                </li>
            </ul>
        </div>
    </nav>
    ...
    <div class="container">
            ...
            <tbody>
                @if(isset($users ) && count($users) > 0)
                    @foreach ($users as $user)
                        <tr>
                            <td>{{ $user['fields']['email']['stringValue'] }}</td>
                            <td>{{ $user['fields']['istri']['stringValue'] ?? 'N/A' }}</td>
                            <td>{{ $user['fields']['suami']['stringValue'] ?? 'N/A' }}</td>
                            <td>{{ $user['fields']['No. Telepon']['stringValue'] ?? 'N/A' }}</td>
                            <td>
                                <ul>
                                    <a href="{{ route('detail', ['email' => $user['fields']['email']['stringValue']]) }}" class="btn btn-info"><i class="fas fa-child"></i> Lihat Detail</a>
                                </ul>
                            </td>
                        </tr>
                    @endforeach
        ...
        <a href="{{ route('perkembangan.create') }}" class="btn btn-custom mt-3"><i class="fas fa-plus"></i> Tambah Perkembangan</a>
    </div>
